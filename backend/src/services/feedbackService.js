      ...
      await notificationEventsService.notifySuperAdmins(
        {
          ...
          message: `${snapshot.userName} أرسل ${typeLabelAr(input.type)}: ${input.subject}`,
          entityType: "feedback",
...
async function listMyFeedback(authUserId, { page = 1, limit = 10 } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT id, type, subject, description, status, created_at, updated_at, reviewed_at, resolved_at
     FROM user_feedback
     WHERE user_id = $1
