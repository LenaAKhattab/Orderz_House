...
(async () => {
  ...
  const plans = (await pool.query(`SELECT tier_code, included_tokens_per_cycle, priority_bid_uses_per_cycle
    FROM marketplace_membership_plans ORDER BY sort_order, id`)).rows;
  const mig139 = (await pool.query(`SELECT version FROM schema_migrations WHERE version LIKE '139%'`)).rows;
  console.log(JSON.stringify({ counts, flags, plans, mig139 }, null, 2));
  await pool.end();
