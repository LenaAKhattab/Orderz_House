/**
 * Marketplace Elite Direct Orders Phase 8 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplaceEliteDirectOrdersPhase8Gate.js
 *
 * NEVER points at Production. Does not git add/commit/deploy/migrate Production.
 * Applies migrations through 143 on the isolated gate DB only.
 */
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_elite_direct_orders_phase8_pg");
const PORT = 55438;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase 8 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function ensureOrdersMinimalSchema(client) {
  ...
  if (!cats.rows[0]) {
    await client.query(`
      ...
      VALUES ('programming', 'Programming', 'Phase 8 gate category')
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS orders (
      ...
      title VARCHAR(200) NOT NULL DEFAULT 'Phase8 Order',
      description TEXT NOT NULL DEFAULT 'Phase 8 gate order description.',
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    // Notifications foundation (Elite notifies after commit; schema helps gate realism)
    try {
      await execSqlFile(
        client,
        path.join(BACKEND_ROOT, "sql", "migrations", "025_notifications.sql"),
      );
    } catch {
      /* optional for gate */
    }
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '143_marketplace_elite_direct_orders'`,
    );
    if (!must.rows[0]) throw new Error("Migration 143 missing after gate bootstrap");
    const table = await client.query(
      `SELECT to_regclass('public.elite_direct_offers') AS o,
              to_regclass('public.elite_direct_order_entitlement_events') AS e`,
    );
    if (!table.rows[0].o || !table.rows[0].e) {
      throw new Error("elite direct order tables missing after 143");
    }
  } finally {
    await client.end();
  }
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-elite-direct-orders-phase8-gate-secret",
    ...
    const child = spawn(
      process.execPath,
      ["--test", "test/marketplaceEliteDirectOrdersPhase8Gate.test.js"],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Elite Direct Orders Phase 8 GATE ===");
  let pg;
  try {
