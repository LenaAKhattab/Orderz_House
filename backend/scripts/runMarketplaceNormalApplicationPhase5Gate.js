/**
 * Marketplace Normal Application Work Token Phase 5 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplaceNormalApplicationPhase5Gate.js
 *
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_normal_app_phase5_pg");
const PORT = 55435;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase 5 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function ensureOrdersMinimalSchema(client) {
  // init.sql already creates categories with NOT NULL description — do not re-seed incompletely.
  const cats = await client.query(`SELECT id FROM categories ORDER BY id LIMIT 1`);
  if (!cats.rows[0]) {
    await client.query(`
      INSERT INTO categories (slug, name, description)
      VALUES ('programming', 'Programming', 'Phase 5 gate category')
      ON CONFLICT DO NOTHING
    `).catch(async () => {
      // Fallback if description column naming differs
      await client.query(`
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS orders (
      ...
      title VARCHAR(200) NOT NULL DEFAULT 'Phase5 Order',
      description TEXT NOT NULL DEFAULT 'Phase 5 gate order description.',
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '140_marketplace_normal_application_work_tokens'`,
    );
    if (!must.rows[0]) throw new Error("Migration 140 missing after gate bootstrap");
    const table = await client.query(
      `SELECT to_regclass('public.order_freelancer_bid_work_token_economics') AS r`,
    );
    if (!table.rows[0].r) throw new Error("economics table missing after 140");
  } finally {
    await client.end();
  }
}
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-normal-app-phase5-gate-secret",
    ...
    const child = spawn(
      ...
      [
        "--test",
        "test/marketplaceNormalApplicationTokenMath.test.js",
        "test/marketplaceNormalApplicationWorkTokenMigration.test.js",
        "test/marketplaceNormalApplicationPhase5Static.test.js",
        "test/marketplaceNormalApplicationPhase5Gate.test.js",
      ],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Normal Application Work Token Phase 5 GATE ===");
  let pg;
  try {
