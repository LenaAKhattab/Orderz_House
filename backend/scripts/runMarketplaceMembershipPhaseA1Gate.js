/**
 * Marketplace Membership Phase A1 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplaceMembershipPhaseA1Gate.js
 *
 * NEVER points at Production. Does not git add/commit/deploy/migrate Production.
 * Applies migrations through 146 on the isolated gate DB only.
 */
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_membership_phase_a1_pg");
const PORT = 55439;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase A1 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '146_marketplace_bid_credits_foundation'`,
    );
    if (!must.rows[0]) throw new Error("Migration 146 missing after gate bootstrap");
    ...
    const bidCol = await client.query(
      `SELECT 1 FROM information_schema.columns
        WHERE table_name = 'marketplace_membership_plans'
          AND column_name = 'article_access_level'`,
    );
    if (!bidCol.rows[0]) throw new Error("article_access_level missing after 144");
  } finally {
    await client.end();
  }
}
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-membership-phase-a1-gate-secret",
    ...
    const child = spawn(
      process.execPath,
      ["--test", "test/marketplaceMembershipPhaseA1Gate.test.js"],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Membership Phase A1 GATE ===");
  let pg;
  try {
