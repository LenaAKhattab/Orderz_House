/**
 * Marketplace Priority Bid Auction Phase 6 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplacePriorityAuctionPhase6Gate.js
 *
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_priority_auction_phase6_pg");
const PORT = 55436;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase 6 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '141_marketplace_priority_bid_auction'`,
    );
    if (!must.rows[0]) throw new Error("Migration 141 missing after gate bootstrap");
    const table = await client.query(
      `SELECT to_regclass('public.priority_bid_auctions') AS a,
              to_regclass('public.priority_auction_bids') AS b`,
    );
    if (!table.rows[0].a || !table.rows[0].b) throw new Error("priority auction tables missing after 141");
  } finally {
    await client.end();
  }
}
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-priority-auction-phase6-gate-secret",
    ...
    const child = spawn(
      ...
      [
        "--test",
        "test/marketplacePriorityAuctionMigration.test.js",
        "test/marketplacePriorityAuctionPhase6Gate.test.js",
      ],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Priority Bid Auction Phase 6 GATE ===");
  let pg;
  try {
