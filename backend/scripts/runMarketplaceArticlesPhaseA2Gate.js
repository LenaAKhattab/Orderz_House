/**
 * Marketplace Articles Phase A2 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplaceArticlesPhaseA2Gate.js
 *
 * NEVER points at Production. Does not git add/commit/deploy/migrate Production.
 * Applies migrations through 145 on the isolated gate DB only.
 */
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_articles_phase_a2_pg");
const PORT = 55440;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase A2 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    // Subcategories required by marketplace_articles FK (from migration 002 on real envs).
    await client.query(`
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '145_marketplace_article_level_model'`,
    );
    if (!must.rows[0]) throw new Error("Migration 145 missing after gate bootstrap");
    const table = await client.query(`SELECT to_regclass('public.marketplace_articles') AS t`);
    if (!table.rows[0].t) throw new Error("marketplace_articles missing after 145");
  } finally {
    await client.end();
  }
}
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-articles-phase-a2-gate-secret",
    ...
    const child = spawn(
      process.execPath,
      ["--test", "test/marketplaceArticlesPhaseA2Gate.test.js"],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Articles Phase A2 GATE ===");
  let pg;
  try {
