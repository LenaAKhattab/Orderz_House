/**
 * Marketplace Fair Distribution Phase 7 — isolated DB gate.
 *
 * Usage (from backend/):
 *   node scripts/runMarketplaceFairDistributionPhase7Gate.js
 *
 * NEVER points at Production. Does not git add/commit/deploy/migrate Production.
 * Applies migrations through 142 on the isolated gate DB only.
 */
...
const DATA_DIR = path.join(BACKEND_ROOT, ".tmp", "marketplace_fair_distribution_phase7_pg");
const PORT = 55437;
...
async function startEmbeddedPostgres() {
  ...
  try {
    ...
    throw new Error(
      "embedded-postgres is required for the Phase 7 gate. Install with: npm install --no-save embedded-postgres@18.4.0-beta.17",
    );
  }
...
async function ensureOrdersMinimalSchema(client) {
  ...
  if (!cats.rows[0]) {
    await client
      ...
      VALUES ('programming', 'Programming', 'Phase 7 gate category')
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS orders (
      ...
      title VARCHAR(200) NOT NULL DEFAULT 'Phase7 Order',
      description TEXT NOT NULL DEFAULT 'Phase 7 gate order description.',
...
async function applyInitAndMigrations(databaseUrl) {
  ...
  try {
    ...
    const must = await client.query(
      `SELECT 1 FROM schema_migrations WHERE version = '142_marketplace_fair_distribution'`,
    );
    if (!must.rows[0]) throw new Error("Migration 142 missing after gate bootstrap");
    const table = await client.query(
      `SELECT to_regclass('public.fair_distribution_decisions') AS d,
              to_regclass('public.fair_distribution_events') AS e`,
    );
    if (!table.rows[0].d || !table.rows[0].e) {
      throw new Error("fair distribution tables missing after 142");
    }
  } finally {
    await client.end();
  }
...
function runGateTests(databaseUrl) {
  return new Promise((resolve) => {
    const env = {
      ...
      JWT_SECRET: process.env.JWT_SECRET || "marketplace-fair-distribution-phase7-gate-secret",
    ...
    const child = spawn(
      ...
      [
        "--test",
        "test/marketplaceFairDistributionPhase7Gate.test.js",
        "test/marketplaceFairDistributionPhase71LiveWiring.test.js",
      ],
...
async function main() {
  // eslint-disable-next-line no-console
  console.log("=== Marketplace Fair Distribution Phase 7 GATE ===");
  let pg;
  try {
