/**
 * Mark all lessons complete for a freelancer except the last two (by sort_order).
 * Usage: node scripts/markCourseProgressExceptLastTwo.js --email=lenakattab@gmail.com --courseId=3
 */
const path = require("path");
require("dotenv").config({ path: path.join(__dirname, "..", ".env") });
const { pool } = require("../src/config/db");

function parseArgs() {
  const out = {};
  for (const raw of process.argv.slice(2)) {
    const m = /^--([^=]+)=(.*)$/.exec(raw);
    if (m) out[m[1]] = m[2];
  }
  return out;
}

async function main() {
  const args = parseArgs();
  const email = String(args.email || "").trim().toLowerCase();
  const courseId = Number(args.courseId);
  if (!email || !Number.isInteger(courseId) || courseId < 1) {
    console.error("Usage: node scripts/markCourseProgressExceptLastTwo.js --email=... --courseId=3");
    process.exit(1);
  }

  const client = await pool.connect();
  try {
    const { rows: users } = await client.query(
      `SELECT id, email, first_name, family_name FROM users WHERE LOWER(email) = $1 LIMIT 1`,
      [email],
    );
    const user = users[0];
    if (!user) {
      console.error(`No user found for email: ${email}`);
      process.exit(1);
    }

    const { rows: lessons } = await client.query(
      `SELECT id, title, sort_order
       FROM course_lessons
       WHERE course_id = $1 AND is_active = TRUE
       ORDER BY sort_order ASC, id ASC`,
      [courseId],
    );

    if (lessons.length < 3) {
      console.error(`Course ${courseId} has fewer than 3 active lessons.`);
      process.exit(1);
    }

    const toComplete = lessons.slice(0, -2);
    const keepOpen = lessons.slice(-2);

    await client.query("BEGIN");

    const { rows: assign } = await client.query(
      `SELECT id FROM course_assignments WHERE course_id = $1 AND freelancer_id = $2 LIMIT 1`,
      [courseId, user.id],
    );
    if (!assign.length) {
      await client.query(
        `INSERT INTO course_assignments (course_id, freelancer_id, assigned_by, assigned_at)
         VALUES ($1, $2, NULL, NOW())
         ON CONFLICT (course_id, freelancer_id) DO NOTHING`,
        [courseId, user.id],
      );
    }

    if (keepOpen.length) {
      await client.query(
        `DELETE FROM course_lesson_progress
         WHERE course_id = $1 AND freelancer_id = $2 AND lesson_id = ANY($3::bigint[])`,
        [courseId, user.id, keepOpen.map((l) => Number(l.id))],
      );
    }

    let inserted = 0;
    for (const lesson of toComplete) {
      const res = await client.query(
        `INSERT INTO course_lesson_progress (course_id, lesson_id, freelancer_id, completed_at)
         VALUES ($1, $2, $3, NOW())
         ON CONFLICT (freelancer_id, course_id, lesson_id) DO NOTHING`,
        [courseId, lesson.id, user.id],
      );
      if (res.rowCount > 0) inserted += 1;
    }

    await client.query(
      `UPDATE course_assignments
       SET completed_at = NULL,
           audit_confirmed = FALSE,
           audit_notes = NULL,
           audit_response_text = NULL,
           audit_response_file_url = NULL,
           audit_submitted_at = NULL
       WHERE course_id = $1 AND freelancer_id = $2`,
      [courseId, user.id],
    );

    await client.query("COMMIT");

    const { rows: after } = await client.query(
      `SELECT COUNT(*)::int AS done FROM course_lesson_progress WHERE course_id = $1 AND freelancer_id = $2`,
      [courseId, user.id],
    );

    console.log(
      JSON.stringify(
        {
          email: user.email,
          userId: String(user.id),
          courseId: String(courseId),
          totalActiveLessons: lessons.length,
          completedTarget: toComplete.length,
          insertedThisRun: inserted,
          leftIncomplete: keepOpen.map((l, i) => ({
            position: lessons.length - 1 + i,
            sortOrder: l.sort_order,
            lessonId: String(l.id),
            title: l.title,
          })),
          totalCompletedNow: after[0]?.done,
          courseAssignmentCompletedAt: null,
        },
        null,
        2,
      ),
    );
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
    console.error(err);
    process.exit(1);
  } finally {
    client.release();
    await pool.end();
  }
}

main();
