/** Mirrors backend type → category mapping for browser push gating. */
...
function getCategoryForType(type) {
  ...
  if (t.startsWith("subscription.")) return "payments";
...
export function shouldShowBrowserPush(notification, prefs) {
  const type = notification?.type;
  const priority = String(notification?.priority || "").toLowerCase();
  if (CRITICAL_TYPES.has(type) || priority === "critical") return true;
...
export function shouldPromptForBrowserNotifications(user) {
  if (!user?.id) return false;
  if (user.notificationPromptAnsweredAt) return false;
  const status = String(user.browserNotificationStatus || "pending").toLowerCase();
