 ...
 * @param {string|number} adId
 */
export function everyLoginSeenStorageKey(userId, loginSessionId, adId) {
  return `${EVERY_LOGIN_KEY_PREFIX}${userId}_${loginSessionId}_${adId}`;
}
 ...
 * @param {{ userId?: string|number|null, loginSessionId?: string|null, isAuthenticated?: boolean }} [options]
 */
export function isPopupAdDismissed(ad, pathname, { userId = null, loginSessionId = null, isAuthenticated = false } = {}) {
  ...
  if (freq === "every_login") {
    if (!isAuthenticated || userId == null || userId === "" || !loginSessionId) return true;
    return sessionStorage.getItem(everyLoginSeenStorageKey(userId, loginSessionId, id)) === "1";
  }
 ...
 * @param {import("../types/popupAd.js").PopupAd} ad
 * @param {string} pathname
 * @param {{ userId?: string|number|null, loginSessionId?: string|null }} [options]
 */
export function markPopupAdDismissed(ad, pathname, { userId = null, loginSessionId = null } = {}) {
  if (!ad?.id) return;
  const id = String(ad.id);
  const freq = ad.frequency || "session";
  const path = normalizePath(pathname);
  ...
  if (freq === "every_login") {
    if (userId == null || userId === "" || !loginSessionId) return;
    sessionStorage.setItem(everyLoginSeenStorageKey(userId, loginSessionId, id), "1");
    return;
  }
 ...
 * @param {{ userId?: string|number|null, loginSessionId?: string|null, isAuthenticated?: boolean }} [options]
 */
export function pickPopupAdToShow(ads, pathname, { userId = null, loginSessionId = null, isAuthenticated = false } = {}) {
  if (!Array.isArray(ads) || !ads.length) return null;
  for (const ad of ads) {
    if (requiresAuthenticatedUser(ad.frequency) && (!isAuthenticated || userId == null || userId === "")) {
      continue;
    }
    if (ad.frequency === "every_login" && !loginSessionId) {
      continue;
    }
    if (!isPopupAdDismissed(ad, pathname, { userId, loginSessionId, isAuthenticated })) return ad;
  }
  return null;
}
