...
import { createClientFixedOrderStripeCheckoutRequest } from "../../services/api";
import { isClientFixedAwaitingPayment, isFixedPoolClaimReviewPhase } from "../../utils/orderFlowUi";
...
function clientStatusMeta(order) {
  ...
  if (ps === "skipped_by_admin") {
    return { label: "تم تجاوز الدفع بواسطة الإدارة", className: "oh-badge oh-badge--neutral" };
  }
  if (ps === "failed") {
    return { label: "فشل الدفع", className: "oh-badge oh-badge--danger" };
  }
  if (s === "pending_payment") {
    return { label: "بانتظار الدفع", className: "oh-badge oh-badge--warning" };
  }
  if (s === "awaiting_payment_after_bid_selection") {
    return { label: "بانتظار الدفع لتفعيل المشروع", className: "oh-badge oh-badge--warning" };
  }
  if (s === "open_for_bids") {
    return { label: "بانتظار العروض", className: "oh-badge oh-badge--warning" };
  }
  if (s === "open_for_freelancers") {
    return { label: "متاح للمستقلين", className: "oh-badge oh-badge--warning" };
  }
  ...
  if (order?.assignedFreelancerId && s === "in_progress") {
    return { label: "جاهز للعمل / قيد التنفيذ", className: "oh-badge oh-badge--success" };
  }
  if (order?.assignedFreelancerId) return { label: "مُسند لمستقل", className: "oh-badge oh-badge--success" };
  if (s === "published" && order?.projectType === "fixed" && order?.sourceType === "client_created" && ps === "paid") {
    return { label: "متاح للمستقلين", className: "oh-badge oh-badge--warning" };
  }
...
export default function ClientOrderCardCompact({ order, onOrdersChange }) {
  ...
  const showClaimsButton = isClientOrder && isFixedPoolClaimReviewPhase(order) && !order?.isArchived;
  const showPayCheckoutButton = isClientOrder && isClientFixedAwaitingPayment(order) && !order?.isArchived;
  ...
  return (
    <article className="client-order-compact">
      ...
      <OrderDeliveryTimingBanner order={order} className="client-order-compact__timing" />
        ...
        {showPayCheckoutButton ? (
          <button
            ...
            onClick={async () => {
              setPayBusy(true);
              try {
                const res = await createClientFixedOrderStripeCheckoutRequest(order.id);
                const url = res?.data?.checkoutUrl;
                if (url) window.location.href = url;
                else
                  push({
                    type: "error",
                    title: "تعذر فتح الدفع",
                    message: "لم يُرجع الخادم رابط الدفع. تحقق من إعداد Stripe أو حاول لاحقاً.",
                  });
              } catch (e) {
                push({
                  type: "error",
          ...
          >
            {payBusy ? "جاري التحضير…" : "إتمام الدفع عبر Stripe"}
          </button>
        ) : null}
        {showClaimsButton ? (
